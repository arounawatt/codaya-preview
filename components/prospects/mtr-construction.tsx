...
export default function MTRMacon() {
  ...
  const services = [
    { icon: '🏗️', title: 'Maçonnerie Générale', desc: "Construction, rénovation et tous travaux de maçonnerie. Savoir-faire artisanal reconnu pour des réalisations durables et de qualité." },
    { icon: '🏠', title: 'Rénovation Complète', desc: "Rénovation d'appartements et maisons du sol au plafond. Travaux clé en main, du gros œuvre aux finitions." },
    { icon: '🧱', title: 'Murs de Soutènement', desc: "Construction de murs de soutènement solides et durables. Solutions adaptées à votre terrain et vos aménagements extérieurs." },
    { icon: '💧', title: 'Drainage & Assainissement', desc: "Terrassement, drainage et assainissement. Protection efficace de vos fondations contre les infiltrations d'eau." },
    { icon: '🛁', title: 'Salle de Bain & Carrelage', desc: "Rénovation complète de salles de bain. Pose de carrelage, faïence et finitions soignées par des professionnels." },
    { icon: '🎨', title: 'Peinture & Parquet', desc: "Travaux de finition : peinture intérieure et pose de parquet dans les règles de l'art. Résultat impeccable garanti." },
  ]

  const whyUs = [
    { icon: '⭐', title: 'Note parfaite 5/5', desc: "20 avis Google avec une note parfaite. La satisfaction de nos clients témoigne de notre sérieux." },
    { icon: '🤝', title: 'Équipe de confiance', desc: "Une équipe jeune, sérieuse et professionnelle. Communication fluide du devis à la livraison." },
    { icon: '✅', title: 'Travail soigné', desc: "Finitions impeccables et respect des règles de l'art. Chaque chantier est réalisé avec le plus grand soin." },
  ...
  const trustItems = [
    { icon: '⭐', title: 'Note 5/5', desc: "Satisfaction client maximale" },
    { icon: '📝', title: '20 avis Google', desc: "Tous positifs et vérifiés" },
    { icon: '🏗️', title: 'Multi-compétences', desc: "Maçonnerie, rénovation, finitions" },
    { icon: '📍', title: 'Essonne & IDF', desc: "Intervention rapide" },
  ]

  const expertise = [
    { icon: '🧱', name: 'Maçonnerie' },
    { icon: '🏠', name: 'Rénovation' },
    { icon: '⛏️', name: 'Terrassement' },
    { icon: '🚿', name: 'Salle de bain' },
    { icon: '🎨', name: 'Peinture' },
    { icon: '🪵', name: 'Parquet' },
  ]
  ...
  const s: Record<string, CSSProperties> = {
    ...
    hero: {
      minHeight: '100vh',
      background: `linear-gradient(135deg, ${colors.primary} 0%, ${colors.primaryLight} 50%, #0f4a5f 100%)`,
      position: 'relative',
      display: 'flex',
      alignItems: 'center',
    ...
    expertiseGrid: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' },
    expertiseItem: {
      display: 'flex',
    ...
    expertiseIcon: { fontSize: '2rem' },
    expertiseName: { fontWeight: 600, color: colors.primary, margin: 0, fontSize: '0.85rem' },
    ...
    whyUsOverlay: {
      ...
      right: 0,
      bottom: 0,
      background: `radial-gradient(circle at 10% 90%, rgba(20, 184, 166, 0.2) 0%, transparent 40%),
    ...
    floatingCtaLink: {
      ...
      textDecoration: 'none',
      fontWeight: 700,
      boxShadow: '0 8px 30px rgba(20, 184, 166, 0.5)',
    },
  }
  ...
  const responsiveStyles = `
    ...
    .mtr-floating-cta { animation: pulse 2s infinite; }
    .mtr-floating-badge { animation: float 3s ease-in-out infinite; }

    @media (max-width: 1024px) {
      .mtr-hero-container { grid-template-columns: 1fr !important; text-align: center; }
      .mtr-hero-visual { display: none !important; }
      .mtr-trust-container { grid-template-columns: repeat(2, 1fr) !important; }
      .mtr-services-grid { grid-template-columns: repeat(2, 1fr) !important; }
      .mtr-why-grid { grid-template-columns: repeat(2, 1fr) !important; }
      .mtr-testimonials-grid { grid-template-columns: 1fr !important; }
      .mtr-contact-container { grid-template-columns: 1fr !important; }
      .mtr-footer-main { grid-template-columns: 1fr !important; gap: 2rem !important; }
    }

    @media (max-width: 768px) {
      .mtr-nav { display: none !important; }
      .mtr-hero { padding: 7rem 1.5rem 3rem !important; }
      .mtr-hero-ctas { flex-direction: column !important; }
      .mtr-trust-container { grid-template-columns: 1fr !important; }
      .mtr-services-grid { grid-template-columns: 1fr !important; }
      .mtr-why-grid { grid-template-columns: 1fr !important; }
  ...
  return (
    <div style={s.wrapper}>
      ...
      <header style={s.header}>
        ...
        <nav style={s.nav} className="mtr-nav">
      ...
      {/* Hero */}
      <section style={s.hero} className="mtr-hero">
        <div style={s.heroOverlay} />
        <div style={s.heroContainer} className="mtr-hero-container">
          <div>
            <div style={s.badge}>🏗️ Expert maçonnerie en Essonne</div>
            <h2 style={s.heroTitle}>
              Votre projet <span style={s.heroTitleAccent}>construction</span> entre de bonnes mains
            </h2>
            <p style={s.heroSubtitle}>
              Maçonnerie, rénovation complète et travaux de finition. Une équipe jeune, sérieuse et professionnelle en {config.zone}.
            </p>
            ...
            <div style={s.heroCtas} className="mtr-hero-ctas">
          ...
          <div style={s.heroVisual} className="mtr-hero-visual">
            <div style={s.heroCard}>
              <div style={s.heroCardBorder} />
              <div style={s.floatingBadge} className="mtr-floating-badge">5/5 ⭐</div>
              <h3 style={{ margin: '0 0 1.5rem', color: colors.primary, fontSize: '1.25rem' }}>Nos expertises</h3>
              <div style={s.expertiseGrid}>
                {expertise.map((item, i) => (
                  <div key={i} style={s.expertiseItem}>
                    <span style={s.expertiseIcon}>{item.icon}</span>
                    <p style={s.expertiseName}>{item.name}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section style={s.trustSection}>
        <div style={s.trustContainer} className="mtr-trust-container">
      ...
      <section style={s.services} id="services">
        <div style={s.sectionHeader}>
          ...
          <h2 style={s.sectionTitle}>Tous vos travaux de construction et rénovation</h2>
          <p style={s.sectionDesc}>Du gros œuvre aux finitions, nous réalisons l&apos;ensemble de vos projets avec soin et professionnalisme.</p>
        </div>
        <div style={s.servicesGrid} className="mtr-services-grid">
      ...
      <section style={s.whyUs} id="pourquoi">
        ...
        <div style={s.whyUsContainer}>
          <div style={s.sectionHeader}>
            ...
            <p style={{ ...s.sectionDesc, ...s.sectionDescLight }}>La qualité et le sérieux reconnus par 20 clients satisfaits.</p>
          </div>
          <div style={s.whyGrid} className="mtr-why-grid">
            {whyUs.map((item, i) => (
              <AnimatedCard key={i} delay={i * 0.1}>
                <div style={s.whyCard}>
      ...
      <section style={s.testimonials} id="avis">
        <div style={s.testimonialsContainer}>
          <div style={s.sectionHeader}>
            ...
            <p style={s.sectionDesc}>20 avis Google — Note parfaite de 5/5 ⭐</p>
          </div>
          <div style={s.testimonialsGrid} className="mtr-testimonials-grid">
      ...
      <section style={s.contact} id="contact">
        <div style={s.contactContainer} className="mtr-contact-container">
          <div>
            <h2 style={s.contactTitle}>Un projet de travaux ?</h2>
            <p style={s.contactDesc}>Contactez-nous pour un devis gratuit et personnalisé. Nous intervenons en Essonne et Île-de-France pour tous vos projets de maçonnerie et rénovation.</p>
            <div style={s.contactMethods}>
              ...
              <div style={s.contactMethod}>
                <div style={s.contactMethodIcon}>📍</div>
                <div>
                  <h4 style={s.contactMethodTitle}>Adresse</h4>
                  <p style={s.contactMethodValue}>{config.zone}</p>
                  <span style={s.contactMethodNote}>{config.address}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={s.contactFormWrapper}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={s.formGroup}>
                <label style={s.formLabel}>Décrivez votre projet</label>
                <textarea
                  placeholder="Type de travaux, surface, délai souhaité..."
                  required
      ...
      <footer style={s.footer}>
        <div style={s.footerContainer}>
          <div style={s.footerMain} className="mtr-footer-main">
            <div>
              <div style={s.logo}>
                <div style={s.logoIcon}>🏗️</div>
                <div>
              ...
              <p style={s.footerBrandDesc}>Expert en maçonnerie, rénovation et travaux de finition. Une équipe jeune et professionnelle pour tous vos projets en {config.zone} et Île-de-France.</p>
            </div>
            <div>
            ...
            <div>
              ...
              <ul style={s.footerLinksList}>
                ...
                <li><span style={s.footerLink}>{config.address}</span></li>
              </ul>
            </div>
          </div>
      ...
      {/* Floating CTA */}
      <div style={s.floatingCta} className="mtr-floating-cta">
